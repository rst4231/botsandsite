...
import { isGroupPhotoAuthError, makeStoryUploadParams, makeWallPostUrl } from '../../../../lib/vk-republish.js';
...
async function uploadWallImages(images, token) {
  ...
  for (let index = 0; index < images.length; index += 1) {
    ...
    const saved = await callVk('photos.saveWallPhoto', { group_id: VK_GROUP_ID, photo: uploaded.photo, server: uploaded.server, hash: uploaded.hash }, token);
...
async function uploadMessageImages(images, token) {
  const attachments = [];
  for (let index = 0; index < images.length; index += 1) {
    ...
    const uploadResponse = await fetch(server.upload_url, { method: 'POST', body: form });
    const uploaded = await uploadResponse.json();
    if (!uploadResponse.ok) throw new Error(`VK message photo upload failed for slide ${index + 1}`);
    const saved = await callVk('photos.saveMessagesPhoto', { photo: uploaded.photo, server: uploaded.server, hash: uploaded.hash }, token);
    const photo = Array.isArray(saved) ? saved[0] : null;
    if (!photo?.owner_id || !photo?.id) throw new Error(`VK failed to save message photo ${index + 1}`);
    attachments.push(`photo${photo.owner_id}_${photo.id}`);
  }
  return attachments;
}

async function uploadPostImages(images, token) {
  try {
    return { attachments: await uploadWallImages(images, token), mode: 'wall-photo' };
  } catch (error) {
    if (!isGroupPhotoAuthError(error)) throw error;
    console.error('VK wall photo upload is unavailable for group token; using message photo storage:', error);
    return { attachments: await uploadMessageImages(images, token), mode: 'message-photo' };
  }
}
...
async function publishStory(storyImage, token, postId) {
  try {
    const result = await uploadStoryVariant(storyImage, token, makeStoryUploadParams(VK_GROUP_ID, postId));
    return { ...result, linkedToPost: true, mode: 'post-sticker' };
  } catch (firstError) {
    console.error('VK story post-sticker mode failed:', firstError);
  }
  try {
    const result = await uploadStoryVariant(storyImage, token, { group_id: VK_GROUP_ID, add_to_news: 1, link_text: 'view', link_url: makeWallPostUrl(VK_GROUP_ID, postId) });
...
export async function GET(request) {
  ...
  if (process.env.PUBLISHING_ENABLED === 'false') return Response.json({ ok: false, error: 'Publishing is disabled' }, { status: 409 });
  ...
  try {
    ...
    const upload = await uploadPostImages(slideImages, token);
    if (upload.attachments.length !== item.slides.length) throw new Error('Not all VK slide images were uploaded');

    // Publish the replacement first. If VK rejects these attachments, the old post stays untouched.
    const posted = await callVk('wall.post', { owner_id: `-${VK_GROUP_ID}`, from_group: 1, message: vkText(item), attachments: upload.attachments.join(',') }, token);
    ...
    try {
      ...
      try {
        await callVk('wall.delete', { owner_id: `-${VK_GROUP_ID}`, post_id: newPostId }, token);
      } catch (rollbackError) {
        console.error('VK rollback failed:', rollbackError);
      }
      throw deleteError;
    }

    status.vk = newPostId;
    status.vkImagesAttached = upload.attachments.length;
    status.vkImageUploadMode = upload.mode;
    ...
    return Response.json({
      ...
      imagesAttached: upload.attachments.length,
      imageUploadMode: upload.mode,
    ...
    console.error(error);
    return Response.json({ ok: false, error: error instanceof Error ? error.message : 'VK replace flow failed', oldPostId, telegramPublished: Boolean(status.telegram) }, { status: 500 });
  }
}
