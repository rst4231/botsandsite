...
import { publicationKindForDate, publishedTopicPostForDate, sendToVk } from '../../../../lib/content-bot.js';
...
export async function GET(request) {
  ...
  try {
    const post = await publishedTopicPostForDate(info.kind, dateKey);
    if (!post) {
      return Response.json({ ok: false, error: 'Published Telegram post could not be reconstructed', kind: info.kind }, { status: 409 });
    }
    const postId = await sendToVk(post.text);
    return Response.json({ ok: true, dateKey, kind: info.kind, postId, title: post.title, telegramMessageId: post.messageId });
  } catch (error) {
    console.error(error);
