...
async function callVk(method, params, token) {
  const body = new URLSearchParams({ ...Object.fromEntries(Object.entries(params).map(([key, value]) => [key, String(value)])), access_token: token, v: VK_API_VERSION });
...
function storyNode(item) {
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '46px' } },
...
export async function GET(request) {
  ...
  if (item.format !== 'slides' || !Array.isArray(item.slides) || item.slides.length !== 5) return Response.json({ ok: false, error: 'Prepared VK story requires five slides', dateKey }, { status: 409 });
  ...
  try {
    const nextStatus = await ensureVkPreparedPublished({
      ...
      publishWall: async () => {
        const post = await apiCall('wall.post', { owner_id: `-${VK_GROUP_ID}`, from_group: 1, message: buildVkPreparedText(item, VK_FOOTER) });
