...
export async function ensureVkPreparedPublished({ item, status = {}, publishWall, publishStory, persist }) {
  if (typeof publishWall !== 'function' || typeof publishStory !== 'function' || typeof persist !== 'function') throw new Error('VK publication callbacks are required');
  const next = { ...status };
  if (!next.vk) { next.vk = await publishWall(item); await persist(next); }
  if (!next.vkStory) { next.vkStory = await publishStory(item, next.vk); await persist(next); }
  return next;
}
...
export async function uploadVkStory({ groupId, image, apiCall, fetchImpl = fetch, linkUrl = '', linkText = 'more' }) {
  ...
  const story = Array.isArray(saved?.items) ? saved.items[0] : Array.isArray(saved) ? saved[0] : saved?.story || saved;
  const storyId = Number(story?.id);
  if (!Number.isInteger(storyId) || storyId <= 0) throw new Error('VK story id is missing');
  return { ownerId: Number(story?.owner_id ?? -Number(groupId)), storyId };
}
