...
export async function sendPreparedPreview({ token, chatId, item, renderSlide, fetchImpl = fetch }) {
  ...
  if (images.length !== 5 || images.some((image) => !image || typeof image.byteLength !== 'number' || image.byteLength <= 0)) {
    throw new Error('Prepared preview requires five successfully rendered images');
  }
  ...
  if (messageIds.length !== 5) throw new Error(`Telegram preview returned ${messageIds.length} messages instead of 5`);
  return messageIds;
}
