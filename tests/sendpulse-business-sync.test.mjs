...
function fakeClient({ destination, source, failTags = false, failVariables = false } = {}) {
  ...
  return {
    ...
    async getContactByTelegramId(botId, telegramId) {
      calls.push(['getContactByTelegramId', botId, String(telegramId)]);
      return source;
    },
    async setVariables(contactId, variables) {
      calls.push(['setVariables', contactId, structuredClone(variables)]);
      if (failVariables) throw new Error('variable write failed');
      return true;
    },
    async setTags(contactId, tags) {
      calls.push(['setTags', contactId, [...tags]]);
      if (failTags) throw new Error('tag write failed');
      return true;
    },
  };
}
...
test('extracts only writable custom variable values and excludes sync marker', () => {
  assert.deepEqual(customVariablesFromContact({
    variables: {
      NAME: 'Artem',
      AGE: 31,
      Active: false,
...
test('ignores unsupported service, title, and another bot', async () => {
  for (const current of [
    event({ service: 'instagram' }),
    event({ title: 'outgoing_message' }),
    event({ bot: { id: 'another-bot' } }),
  ]) {
    const client = fakeClient();
...
test('skips an already synchronized Business contact', async () => {
  const client = fakeClient({
    destination: {
      id: 'business-contact',
      telegram_id: '123456789',
...
test('ordinary bot incoming message is not treated as a Business contact', async () => {
  const client = fakeClient({
    destination: { id: 'normal-contact', telegram_id: '123456789', variables: {} },
    source: { id: 'normal-contact', telegram_id: '123456789', variables: { NAME: 'Artem' } },
  });
  const result = await processBusinessSyncEvent(
    event({ contact: { id: 'normal-contact' } }),
    { enabled: true, botId: BOT_ID, client },
  );
...
test('copies every custom variable and unique tag, then writes marker last', async () => {
  const client = fakeClient({
    source: {
      id: 'normal-contact',
      telegram_id: '123456789',
      variables: {
        ...
        AGE: 31,
        Goal: 'profit',
        Business_sync: 'old-value',
      },
      tags: ['купил', 'лид', 'купил'],
    },
  });
  ...
  assert.deepEqual(result, {
    status: 'success',
    destinationId: 'business-contact',
    sourceId: 'normal-contact',
  ...
  assert.deepEqual(writes, [
    ...
    ['setVariables', 'business-contact', [
      { variable_name: 'Business_sync', variable_value: 'done' },
    ]],
  ]);
});
...
test('does not write marker after tag copy failure', async () => {
  ...
  await assert.rejects(
    processBusinessSyncEvent(event(), { enabled: true, botId: BOT_ID, client }),
    /tag write failed/,
  );

  const markerWrites = client.calls.filter(([name, , variables]) =>
    name === 'setVariables' && variables?.some((variable) => variable.variable_name === 'Business_sync'));
...
test('REST client uses exact Telegram contact endpoints and request bodies', async () => {
  ...
  const fetchImpl = async (url, init = {}) => {
    ...
    return jsonResponse({ success: true, data: { id: 'ok' } });
  };
  ...
  assert.deepEqual(JSON.parse(calls[2].init.body), {
    ...
    variables: [{ variable_name: 'NAME', variable_value: 'Artem' }],
  });
...
test('REST client obtains one OAuth token and reuses it', async () => {
  ...
  const fetchImpl = async (url, init = {}) => {
    ...
    if (String(url).endsWith('/oauth/access_token')) {
      return jsonResponse({ access_token: 'oauth-token', token_type: 'Bearer', expires_in: 3600 });
    }
  ...
  const oauthBody = JSON.parse(calls[0].init.body);
  assert.deepEqual(oauthBody, {
