...
const transform = [
  ...
  "    \"  try {\",",
  "    \"    const loaded = await loadRuntimeContentIssue(dateKey);\",",
  "    \"    if (!loaded?.item) return null;\",",
  "    \"    const normalizedItem = validatePreparedItem(loaded.item);\",",
  "    \"    const cachedItem = { ...normalizedItem, ...(loaded.item.fingerprint ? { fingerprint: String(loaded.item.fingerprint) } : {}), stagedAt: loaded.item.stagedAt || loaded.updatedAt || new Date().toISOString() };\",",
  "    \"    await cache.set(`prepared-content:${dateKey}`, cachedItem, { ttl: 60 * 60 * 24 * 7, tags: [\'prepared-content\'] });\",",
  "    \"    if (loaded.status && Object.keys(loaded.status).length) {\",",
  "    \"      const existingStatus = await cache.get(`prepared-status:${dateKey}`);\",",
  "    \"      if (!existingStatus) await cache.set(`prepared-status:${dateKey}`, loaded.status, { ttl: CACHE_TTL, tags: [\'prepared-publications\'] });\",",
  "    \"    }\",",
  "    \"    return { ...loaded, item: cachedItem };\",",
  "    \"  } catch (error) {\",",
